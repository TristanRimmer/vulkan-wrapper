...
auto DescriptorUtils::DescriptorContainer::create(
  ...
  for (auto fif = 0; fif < num_frames_in_flight; fif++) {
    ...
    auto num_ubos = 0;
    auto num_combined_image_samplers = 0;
    auto num_image_samplers = 0;
    ...
    for (auto binding_i = 0; binding_i < binding_orders.size(); binding_i++) {
      ...
      case Enum::Uniform_Buffer: {
        buffer_infos.emplace_back(vk::DescriptorBufferInfo{
            .buffer = descriptor_informer.get_ubo_buffers_ref(num_ubos)[fif],
            .offset = 0,
            .range = descriptor_informer.get_ubo_buffer_size(num_ubos)});
      ...
      case Enum::Combined_Image_Sampler: {
        ...
        auto combined = descriptor_informer.get_combined_image_sampler_ref(
            num_combined_image_samplers);
        auto &sampler = combined.first.get();
        auto &image_view = combined.second.get();
      ...
      case Enum::Sampler:
        sampler_and_or_image_infos.emplace_back(vk::DescriptorImageInfo{
            .sampler =
                descriptor_informer.get_sampler_ref(num_image_samplers)});
